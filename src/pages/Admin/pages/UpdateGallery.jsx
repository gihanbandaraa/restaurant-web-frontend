...
const UpdateGallery = () => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then(async (imageUrl) => {
          try {
            ...
            if (res.ok) {
              ...
              handleShowAlert("error", data.message || "Failed to store the image link.");
            }
          } catch (error) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setImageIds((prev) => prev.filter((imageId) => imageId !== deletingImage.id));
  ...
  return (
    <section className="fixed-container p-6 bg-white shadow-lg rounded-lg">
      <Alert
        type={alertInfo.type}
        ...
        {imageUrls.length > 0 ? (
          imageUrls.map((imageUrl, index) => (
            ...
            >
              ...
              >
                <FaTrash
                  onClick={() => openConfirmDeleteModal(imageIds[index], imageUrl)}
